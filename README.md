...
Each problem is saved in a separate file named 1.txt, 2.txt, and so on to 20.txt.
The job application number is divided by 20 and the remainder is stored. Then 1 is added to the remainder to find the name of the file from which the Problem statement is to be fetched.
The contents of the file are then displayed on the browser. You have to create a website on the above scenario and host it on the AWS Server and your local Machine using WAMP/XAMPP server. Use File handling or Database as per your choice to store the data.
Checklist:

All the fields are to be filled before clicking the submit button.
The look and feel should be self explanatory.
The Company name should also be displayed.
The text area where the code is to be written must have sufficient width and height.
The file to be saved on the server shall have the extension corresponding to the language chosen.
